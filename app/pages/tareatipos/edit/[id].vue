...
const getItem = async () => {
  ...
  try {
    ...
    let res = await cliente.get(`api/tarea-tipos/${id}`);
...
async function onSubmit(event: FormSubmitEvent<InferType<typeof schema>>) {
  ...
  try {
    ...
    let res = await cliente.put(`api/tarea-tipos/${id}`, state);

    notifySuccess('TareaTipo Actualizad@', res.data.message);

    navigateTo('/tareatipos');
