...
const getItem = async () => {
  ...
  try {
    ...
    let res = await cliente.get(`api/tarea-estados/${id}`);
...
async function onSubmit(event: FormSubmitEvent<InferType<typeof schema>>) {
  ...
  try {
    ...
    let res = await cliente.put(`api/tarea-estados/${id}`, state);

    notifySuccess('TareaEstado Actualizad@', res.data.message);

    navigateTo('/tareaestados');
