...
const getItem = async () => {
  ...
  try {
    ...
    let res = await cliente.get(`api/tarea-prioridades/${id}`);
...
async function onSubmit(event: FormSubmitEvent<InferType<typeof schema>>) {
  ...
  try {
    ...
    let res = await cliente.put(`api/tarea-prioridades/${id}`, state);

    notifySuccess('TareaPrioridad Actualizad@', res.data.message);

    navigateTo('/tareaprioridades');
    ...
    <template #footer>
      <div>
        <UButton type="button"
                 ...
                 variant="solid"
                 label="Regresar"
                 @click="navigateTo('/tareaprioridades')"
                 class="mr-1"
        />
        <UButton type="button"
                 label="Guardar"
